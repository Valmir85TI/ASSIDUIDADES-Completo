...
class natal():
    try:
        def funcao_a_executar2(self):
            # Cria janela principal oculta apenas para gerenciar a janela de loading
            root = Tk()
            root.withdraw()
            ...
            # Cria janela de loading centralizada
            loading = Toplevel()
            loading.iconbitmap('icone.ico')
            loading.title("Gerando Documentos")
            loading.geometry(f"{largura}x{altura}+{x}+{y}")
            loading.resizable(False, False)
            ttk.Label(loading, text="Processando documentos...").pack(pady=10)
            progresso = ttk.Progressbar(loading, orient='horizontal', length=300, mode='determinate')
            ...
            for dado in range(quant):
                arquivo = f"""      Supermercado Castanha LTDA., inscrita no CNPJ/MF: 63.082.721/0001-08, empregador, através do presente termo faz registrar a concessão do VALE NATALINO na forma prevista pelo parágrafo único da cláusula 65ª da CCT 2025/2026, mediante os critérios abaixo especificados:

        Conforme previsto no parágrafo único da cláusula 65ª da CCT 2025/2026, a empresa poderá substituir a Cesta Natalina pelo documento-refeição como vale natalino com o valor correspondente aos produtos contemplados na cesta de natal.
        Conforme apurado, os produtos contemplados na cesta de natal equivale ao valor de R$110,00 (cento e dez reais).
        Todos os funcionários serão contemplados com o recebimento do vale compra (documento-refeição) no valor de R$110,00 (cento e dez reais) a título de vale natalino.

        O vale compra (documento-refeição) a título de vale natalino no valor de R$110 (cento e dez reais) somente poderá ser utilizado para aquisição de produtos comercializados pelo empregador.

        Diante do exposto, consensualmente fica celebrado:
        O funcionário declara que está recebendo o vale compra (documento-refeição) no valor de R$110 (cento e dez reais) a título de vale natalino em substituição à Cesta Natalina conforme previsto no parágrafo único da cláusula 65ª da CCT 2025/2026.
        ...
        São Paulo, {data}.
            ...
            {NOME[dado]}\n"""
                ...
                run = paragraph.add_run(
                    '                                 TERMO DE CONCESSÃO DO VALE NATALINO\n              Parágrafo Único da Cláusula 65ª da Convenção Coletiva 2025/2026\n')
